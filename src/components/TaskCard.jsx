...
const TaskCard = forwardRef(({ task, subject, onToggleDone, onEdit, onDelete, onTogglePin, onViewDetails }, ref) => {
  ...
  return (
    <motion.div
      ...
      className={`task-card interactive bg-white rounded-xl border-[2.5px] border-gray-300 shadow-[var(--shadow-sm)] p-4 transition-all duration-500 dark:bg-[var(--dm-surface)] dark:border-[var(--dm-border)] dark:shadow-[0_0_0_1px_rgba(255,255,255,0.02)] ${task.done ? 'done' : ''}`}
      style={task.done ? {
    ...
    >
      <div className="flex items-start gap-3">
        <button
          ...
          className={`task-checkbox mt-1 w-5 h-5 rounded border-2 flex items-center justify-center ${task.done ? 'bg-green-500 border-green-500 text-white' : 'border-gray-300 hover:border-blue-500 dark:border-[var(--dm-border)] dark:hover:border-[var(--color-primary)]'}`}
        >
          {task.done && (
        ...
        <div className="flex-1">
          <div className="flex items-center justify-between">
            <h4 className={`task-title font-medium ${task.done ? 'line-through' : ''}`} style={task.done ? { color: 'var(--text3)', transition: 'all 0.3s ease' } : { color: 'var(--dm-text)' }}>
              {task.titulo}
            </h4>
            {badgeConfig && (
