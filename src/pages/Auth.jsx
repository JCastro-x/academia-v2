import { useState, useEffect } from 'react'
import { useNavigate } from 'react-router-dom'
import { signInWithGoogle } from '../lib/supabase.js'
import { useUIStore } from '../stores/ui.store.js'

export default function Auth() {
  const [isGuest, setIsGuest] = useState(false)
  const navigate = useNavigate()
  const resetTheme = useUIStore(s => s.resetTheme)

  // Resetear tema al salir (por si otro usuario se loguea sin refrescar)
  useEffect(() => { resetTheme() }, [resetTheme])

  const handleGoogleSignIn = async () => {
    try {
      await signInWithGoogle()
      // OAuth redirect will handle the rest
    } catch (error) {
      console.error('Error signing in with Google:', error)
    }
  }

  const handleGuestMode = () => {
    localStorage.setItem('academia-guest-mode', 'true')
    navigate('/s/guest')
  }

  if (isGuest) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="bg-white p-8 rounded-lg shadow-md max-w-md">
          <h2 className="text-2xl font-bold mb-4">Modo Invitado</h2>
          <p className="text-gray-600 mb-6">
            En modo invitado, tus datos se guardan localmente en este navegador.
            No se sincronizarán entre dispositivos y puedes perderlos si borras el caché.
          </p>
          <button
            onClick={handleGuestMode}
            className="w-full bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700"
          >
            Continuar como invitado
          </button>
          <button
            onClick={() => setIsGuest(false)}
            className="w-full mt-4 text-gray-600 hover:text-gray-800"
          >
            Volver
          </button>
        </div>
      </div>
    )
  }

  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="bg-white p-8 rounded-lg shadow-md max-w-md">
        <h1 className="text-3xl font-bold mb-6 text-center">Academia v2</h1>
        
        <button
          onClick={handleGoogleSignIn}
          className="w-full flex items-center justify-center gap-3 bg-white border border-gray-300 py-3 px-4 rounded-lg hover:bg-gray-50 mb-4"
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
          Ingresar con Google
        </button>

        <button
          onClick={() => setIsGuest(true)}
          className="w-full bg-gray-200 text-gray-800 py-3 px-4 rounded-lg hover:bg-gray-300"
        >
          Probar sin cuenta
        </button>

        <div className="mt-6 text-center text-sm text-gray-600">
          <a href="/terms" className="hover:underline">Términos</a>
          {' · '}
          <a href="/privacy" className="hover:underline">Privacidad</a>
        </div>
      </div>
    </div>
  )
}
